...
export default function TrendRadar() {
  ...
  const matchOverlay = selectedMatch && <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#060512]/80 backdrop-blur-md px-4" onClick={() => setSelectedMatch(null)}><div className="glass w-full max-w-xl rounded-3xl p-6" onClick={(event) => event.stopPropagation()}><div className="flex items-start justify-between"><div><p className="mono text-[10px] uppercase tracking-widest text-[#8ea7ff]">AI match intelligence</p><h3 className="display text-2xl font-extrabold mt-1">{selectedMatch.teams?.home?.name} vs {selectedMatch.teams?.away?.name}</h3><p className="body-f text-xs theme-muted mt-1">{selectedMatch.league?.name || "Football"}</p></div><button onClick={() => setSelectedMatch(null)} className="text-xl theme-muted">×</button></div>{matchLoading ? <p className="body-f text-sm theme-muted py-10 text-center">Analysing live match data…</p> : <div className="mt-6 space-y-3"><div className="rounded-2xl bg-[#142650]/70 border border-[#6f8dff]/25 p-4"><p className="mono text-[10px] uppercase tracking-widest text-[#b9d5ff]">AI analysis inputs</p><div className="grid grid-cols-2 gap-3 mt-3 text-sm body-f"><span>Win probabilities</span><span>Expected goals</span><span>Corners and cards</span><span>Bookmaker odds</span></div></div><p className="body-f text-xs theme-muted">Full probabilities are based on live fixture data and statistics. No result is guaranteed.</p></div>}<button onClick={() => toggleMatchSaved(selectedMatch.fixture?.id)} className="mt-5 w-full rounded-xl border border-[#6f8dff]/40 py-3 text-sm font-semibold text-[#b9d5ff]">{savedMatches.has(selectedMatch.fixture?.id) ? "★ Saved to favourites" : "☆ Save match to favourites"}</button></div></div>;
  return (
