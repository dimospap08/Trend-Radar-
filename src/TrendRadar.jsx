...
export default function TrendRadar() {
  ...
  const matchOverlay = selectedMatch && <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#060512]/80 backdrop-blur-md px-4" onClick={() => setSelectedMatch(null)}><div className="glass w-full max-w-xl rounded-3xl p-6" onClick={(event) => event.stopPropagation()}><div className="flex items-start justify-between"><div><p className="mono text-[10px] uppercase tracking-widest text-[#8ea7ff]">AI match intelligence</p><h3 className="display text-2xl font-extrabold mt-1">{selectedMatch.teams?.home?.name} vs {selectedMatch.teams?.away?.name}</h3><p className="body-f text-xs theme-muted mt-1">{selectedMatch.league?.name || "Football"}</p></div><button onClick={() => setSelectedMatch(null)} className="text-xl theme-muted">×</button></div>{matchLoading ? <p className="body-f text-sm theme-muted py-10 text-center">Analysing live match data…</p> : <div className="mt-6 space-y-3"><div className="rounded-2xl bg-[#142650]/70 border border-[#6f8dff]/25 p-4"><p className="mono text-[10px] uppercase tracking-widest text-[#b9d5ff]">AI prediction report</p><div className="grid grid-cols-3 gap-2 mt-3 text-center"><div><p className="mono text-[9px] text-[#9eb9e8]">HOME</p><p className="display text-xl font-extrabold text-[#35d07f]">{matchDetails?.predictions?.predictions?.percent?.home || "—"}</p></div><div><p className="mono text-[9px] text-[#9eb9e8]">DRAW</p><p className="display text-xl font-extrabold text-[#f5b83d]">{matchDetails?.predictions?.predictions?.percent?.draw || "—"}</p></div><div><p className="mono text-[9px] text-[#9eb9e8]">AWAY</p><p className="display text-xl font-extrabold text-[#58b8ff]">{matchDetails?.predictions?.predictions?.percent?.away || "—"}</p></div></div><div className="mt-4 grid grid-cols-2 gap-3 text-sm body-f"><span>Possible score: <b>{matchDetails?.predictions?.predictions?.goals?.home ?? "—"} - {matchDetails?.predictions?.predictions?.goals?.away ?? "—"}</b></span><span>Advice: <b>{matchDetails?.predictions?.predictions?.advice || "Data review"}</b></span></div></div><p className="body-f text-xs theme-muted">Based on live fixture, statistics and odds data. No result is guaranteed.</p></div>}<button onClick={() => toggleMatchSaved(selectedMatch.fixture?.id)} className="mt-5 w-full rounded-xl border border-[#6f8dff]/40 py-3 text-sm font-semibold text-[#b9d5ff]">{savedMatches.has(selectedMatch.fixture?.id) ? "★ Saved to favourites" : "☆ Save match to favourites"}</button></div></div>;
  return (
