...
const NAME_POOL = {
  ...
  Product: ["mini heatless curler v2", "glass-skin serum stick", "LED desk fog lamp", "wearable neck-fan clip", "magnetic phone tripod", "portable espresso maker", "sunset projector lamp", "smart posture trainer", "cordless label printer", "rechargeable hand warmer", "wireless lapel microphone", "compact air purifier", "heated eye mask", "foldable walking pad", "portable blender bottle", "motion sensor night light", "silicone travel organizer", "desktop ice maker", "smart water bottle", "ergonomic laptop stand"],
  ...
  MemeCoin: ["PEPE community revival", "BONK trading velocity", "FLOKI gaming narrative", "BRETT Base momentum", "WIF social dominance", "DOGE payment chatter", "SHIB ecosystem updates", "MOG community growth", "POPCAT attention cycle", "NEIRO exchange activity", "TURBO creator momentum", "SPX6900 social growth", "GOAT AI meme cycle", "MEW community activity", "GIGA holder growth", "MOTHER creator token", "PONKE trading interest", "MIGGLES meme attention", "DOG meme rotation", "Base meme launch cycle"],
  CopyTrader: ["Top crypto futures trader", "Balanced BTC portfolio", "Low-risk altcoin trader", "DeFi swing specialist", "Momentum trader leaderboard", "Consistent monthly performer", "High-conviction crypto trader", "Diversified copy portfolio", "Short-term futures leader", "Long-term trend follower", "Low-drawdown BTC trader", "Top-performing altcoin profile", "Steady algorithmic trader", "Risk-managed futures leader", "Copy portfolio income strategy", "High-win-rate swing trader", "Diversified derivatives trader", "Conservative crypto allocator", "Breakout strategy leader", "Multi-asset copy portfolio"],
  GlobalMarket: ["AI infrastructure spending", "Copper demand cycle", "Freight rates rebound", "Energy storage buildout", "Emerging-market currency rotation", "Defence technology investment", "Global cloud capex", "Renewable grid upgrades", "Semiconductor equipment demand", "Food commodity logistics", "Data-center power demand", "Industrial automation cycle", "Critical minerals supply", "Global tourism recovery", "Healthcare innovation spend", "Digital payments adoption", "Aerospace manufacturing demand", "Water infrastructure investment", "Agricultural technology growth", "Battery materials cycle"],
};
...
const PUBLIC_SIGNAL_FOLDERS = new Set(["CopyTrader", "GlobalMarket"]);
...
export default function TrendRadar() {
  ...
  useEffect(() => {
    ...
    return () => { cancelled = true; };
  }, [hasSignalAccess, session?.access_token, footballDays]);
  ...
  const searchFootball = async () => {
    ...
    setFootballLoading(true);
    try {
      const response = await fetch(`${API_BASE}/api/sports/search?q=${encodeURIComponent(query)}&days=${footballDays}`, { headers: { Authorization: `Bearer ${session.access_token}` } });
...
function MatchAnalytics({ match, loading, details, saved, close, toggleSaved }) {
  ...
  return (
    <div className={`min-h-screen ${lightMode ? "theme-light" : "theme-dark"} relative`}>
      ...
      {!isLoggedIn && <a href="#pricing" className="md:hidden fixed bottom-4 left-4 right-4 z-20 flex items-center justify-center gap-2 rounded-xl bg-[#173b68] py-3.5 text-sm font-bold text-white shadow-[0_8px_30px_rgba(8,30,58,.45)]">Request beta access <ArrowRight className="w-4 h-4" /></a>}
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;600;700&family=Manrope:wght@500;600;700;800&family=Plus+Jakarta+Sans:wght@400;500;600;700&family=Space+Grotesk:wght@400;500;600;700&display=swap');
        .display { font-family: 'Space Grotesk', 'Manrope', sans-serif; letter-spacing: -0.035em; }
        .body-f { font-family: 'Plus Jakarta Sans', 'DM Sans', sans-serif; }
        /* Keep metadata compact without the rigid typewriter look. */
      ...
      <section className="max-w-6xl mx-auto px-6 pb-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3">
          ...
          {SIGNAL_LOCKED_FOLDERS.filter((item) => PUBLIC_SIGNAL_FOLDERS.has(item.category)).map((item) => { const Icon = item.icon; return <button key={item.key} onClick={() => { setSelectedCategory(item.category); document.getElementById("feed")?.scrollIntoView({ behavior: "smooth" }); }} className="group relative glass rounded-2xl border border-[#38bdf8]/35 p-4 text-left hover:-translate-y-1 hover:border-[#58b8ff] transition"><div className="flex h-10 w-10 items-center justify-center rounded-xl mb-3" style={{ background: `${item.color}25` }}><Icon className="h-5 w-5" style={{ color: item.color }} /></div><p className="display text-xs font-bold">{item.key}</p><p className="body-f text-[10px] text-[#9eb9e8] mt-1">Open folder →</p></button>; })}
          {!hasSignalAccess && SIGNAL_LOCKED_FOLDERS.filter((item) => !PUBLIC_SIGNAL_FOLDERS.has(item.category)).map((item) => { const Icon = item.icon; return <button key={item.key} onClick={() => { setSignalNotice(true); window.setTimeout(() => setSignalNotice(false), 4200); }} className="group relative glass rounded-2xl border border-[#c084fc]/25 p-4 text-left hover:-translate-y-1 hover:border-[#c084fc] transition"><div className="absolute right-3 top-3 rounded-full bg-[#0b1028]/80 p-1.5 text-[#c084fc]"><Lock className="h-3.5 w-3.5" /></div><div className="flex h-10 w-10 items-center justify-center rounded-xl mb-3" style={{ background: `${item.color}25` }}><Icon className="h-5 w-5" style={{ color: item.color }} /></div><p className="display text-xs font-bold">{item.key}</p><p className="body-f text-[10px] text-[#9eb9e8] mt-1">Signal plan · locked</p></button>; })}
