  ...
  return (
    <div className={`min-h-screen ${lightMode ? "theme-light" : "theme-dark"} relative`}>
      ...
      <style>{`
        ...
        .display { font-family: 'Outfit', sans-serif; letter-spacing: -0.035em; }
        ...
        * { scrollbar-color: #2a2150 #060512; }
        ...
        .theme-dark::before { content: '✦  777  ✧  $  ✦  888  ✧  $  ✦'; position: fixed; inset: 0; z-index: 0; pointer-events: none; opacity: .035; color: #c084fc; font: 800 22px/5rem 'Outfit', sans-serif; letter-spacing: 1.2rem; transform: rotate(-12deg) scale(1.3); animation: ambientDrift 18s ease-in-out infinite alternate; white-space: pre-wrap; }
        .theme-dark > * { position: relative; z-index: 1; }
        @keyframes ambientDrift { from { transform: rotate(-12deg) translate3d(-2%, -1%, 0) scale(1.3); } to { transform: rotate(-8deg) translate3d(2%, 1%, 0) scale(1.36); } }
        .premium-kicker { letter-spacing: .18em; text-transform: uppercase; font-weight: 800; }
      `}</style>
      ...
      <section className="max-w-6xl mx-auto px-6 pb-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3">
          {CATEGORY_VISUALS.map((item) => { const Icon = item.icon; const isFree = FREE_TRIAL_FOLDERS.has(item.key); return <button key={item.key} onClick={() => { if (!isFree) { setSignalNotice(true); window.setTimeout(() => setSignalNotice(false), 4200); return; } setSelectedCategory(item.key); document.getElementById("feed")?.scrollIntoView({ behavior: "smooth" }); }} className={`relative glass rounded-2xl border p-4 text-left hover:-translate-y-1 transition ${isFree ? "border-[#6f8dff]/25 hover:border-[#58b8ff]" : "border-[#c084fc]/25 hover:border-[#c084fc]"}`}>
